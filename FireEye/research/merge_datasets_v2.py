#!/usr/bin/env python3
"""
Merge datasets v2: improved version with fire extinguisher data,
cleaned labels, and no empty background-only sources.
"""
...
def add_dataset(name, img_dir, lbl_dir, out_img_dir, out_lbl_dir,
                ...
                skip_empty=False):
    """Add a dataset. skip_empty=True drops images with no labels."""
    ...
    for img_path in images:
        ...
        # Check if we should skip empty labels
        if skip_empty:
            has_labels = False
...
def main():
    ...
    parser.add_argument("--output", default="/home/evnchn/Scaffluent_App/FireEye/research/merged_dataset_v2")
    ...
    total = 0
    print("Merging datasets v2 (improved quality)...\n")

    # 1. Synthetic SDXL-Turbo (skip images with no labels)
    ...
    # 2. Construction-PPE (skip images with no remappable labels)
    ...
    # 3. Welding frames (only if they have labels from auto_label_v2)
    ...
    # 4. D-Fire (skip empty, cap per split)
    ...
    # 5. Fire extinguisher targeted data (NEW in v2)
    ...
    # NOTE: Removed earlier synthetic_bulk, augmented_samples, severity_variations
    # because they had zero labels (all empty), adding only noise to training
    ...
    import time
    yaml_content = f"""# FireEye Merged Training Dataset v2
# Generated: {time.strftime('%Y-%m-%d %H:%M')}
# Train: {len(train_imgs)}, Val: {len(val_imgs)}
# Improvements: cleaned labels, fire extinguisher data, no empty backgrounds
...
"""
    ...
    print(f"\n{'='*60}")
    print(f"Merged dataset v2 ready!")
